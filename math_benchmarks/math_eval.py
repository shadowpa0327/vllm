...
def parse_args():
    ...
    parser.add_argument(
        "--vllm_sspec_recent_size",
        type=int,
        default=128,
        help="Number of recent tokens to keep for sparse attention (default: 128)",
    )
    parser.add_argument(
        "--vllm_sspec_sink_size",
        type=int,
        default=32,
        help="Number of sink tokens to keep for sparse attention (default: 32)",
    )
    parser.add_argument(
...
def setup(args):
    ...
    if args.use_sspec:
        ...
        if args.use_vllm:
            ...
            spec_options = sum([args.vllm_enable_ngram, args.vllm_enable_eagle3, args.vllm_enable_sspec])
            if spec_options > 1:
                raise ValueError(
                    "Cannot enable multiple speculative decoding methods at the same time. "
                    "Please choose only one: vllm_enable_ngram, vllm_enable_eagle3, or vllm_enable_sspec."
                )

            # Set environment variables for self-spec (must be set before vLLM import)
            if args.vllm_enable_sspec:
                os.environ["VLLM_USE_V1"] = "1"
                os.environ["VLLM_ENABLE_V1_MULTIPROCESSING"] = "0"
                os.environ["VLLM_ATTENTION_BACKEND"] = "FLASHINFER"
                print("Enabled V1 engine for self-speculative decoding")
                print(f"  VLLM_USE_V1=1")
                print(f"  VLLM_ENABLE_V1_MULTIPROCESSING=0")
            ...
            if args.vllm_enable_sspec:
                ...
                vllm_kwargs["recent_size"] = args.vllm_sspec_recent_size
                ...
                vllm_kwargs["max_num_batched_tokens"] = 8192
                ...
                print(f"  - Recent size (sparse attention): {args.vllm_sspec_recent_size}")
                print(f"  - Sink size (sparse attention): {args.vllm_sspec_sink_size}")
...
def main(llm, tokenizer, data_name, args):
    ...
    # Get vLLM speculation statistics after all generation is done (ngram, EAGLE3, or self-spec)
    if args.use_vllm and (args.vllm_enable_ngram or args.vllm_enable_eagle3 or args.vllm_enable_sspec):
        try:
            if args.vllm_enable_ngram:
                spec_method = "ngram"
