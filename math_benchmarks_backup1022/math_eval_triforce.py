...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.05,
        help="Budget ratio (sparsity parameter) for sspec inference. Controls the fraction of KV cache to keep (e.g., 0.05 = 5%)",
    )
    parser.add_argument(
        "--sspec_kv_util_threshold",
        type=float,
        default=0.0,
        help="KV utilization threshold for disabling speculation permanently. When no pending requests and KV util < threshold, "
        "speculation is disabled. Default 0.0 means disabled.",
    )
    parser.add_argument(
        "--sspec_batch_policy",
        type=str,
        ...
        help="Batch policy for sspec inference (greedy: load balance, random: random assignment, naive: max bucket)",
    )
    parser.add_argument(
        "--enable_wandb",
        action="store_true",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.95,
        help="Fraction of GPU memory to use for vLLM (default: 0.95)",
    )
    parser.add_argument(
        "--vllm_max_model_len",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Maximum number of sequences to process in parallel (default: 128)",
    )
    parser.add_argument(
        "--vllm_enforce_eager",
        action="store_true",
...
def setup(args):
    ...
    if args.use_sspec:
        ...
        if args.use_vllm:
            ...
            print(f"  - Max model length: {args.vllm_max_model_len if args.vllm_max_model_len else 'auto'}")
            ...
            if args.vllm_enable_sspec:
                ...
                # Keep user-specified gpu_memory_utilization instead of overriding
                # vllm_kwargs["gpu_memory_utilization"] = 0.9  # REMOVED: respect user's setting
                vllm_kwargs["max_num_batched_tokens"] = 16384
                # Keep user-specified max_num_seqs instead of overriding
                # vllm_kwargs["max_num_seqs"] = 128  # REMOVED: respect user's setting
            ...
            if args.vllm_enable_sspec_ngram:
                ...
                vllm_kwargs["enable_prefix_caching"] = False
                # Keep user-specified gpu_memory_utilization instead of overriding
                # vllm_kwargs["gpu_memory_utilization"] = 0.95  # REMOVED: respect user's setting
                vllm_kwargs["max_num_batched_tokens"] = 16384
                # Keep user-specified max_num_seqs instead of overriding
                # vllm_kwargs["max_num_seqs"] = 192  # REMOVED: respect user's setting
...
def main(llm, tokenizer, data_name, args):
    ...
    # Initialize wandb for vLLM/SGLang if enabled
    inference_wandb_logger = None
    if (args.use_vllm or args.use_sglang) and args.enable_wandb and WANDB_AVAILABLE:
        inference_wandb_logger = None
        model_name = args.model_name_or_path.split("/")[-1]
        available_gpus = os.environ.get("CUDA_VISIBLE_DEVICES", "0")
        num_gpus = len(available_gpus.split(","))
        engine_name = "sglang" if args.use_sglang else "vllm"
        wandb_config = {
    ...
    for epoch in range(max_func_call):
        ...
        if args.use_vllm:
            ...
            for i, out in enumerate(vllm_outputs):
                ...
                print(f"num outputs: {len(getattr(out.outputs[0], 'token_ids', []))}")
    ...
    with open(
        ...
        if (
            (args.use_vllm or args.use_sglang)
            and total_draft_tokens > 0
            and total_accepted_tokens > 0
        ):
            acceptance_rate = (
            ...
            if args.use_sglang and args.sglang_enable_eagle3:
                ...
                result_json["speculative_decoding"][
                    "note"
                ] = "self-speculative decoding with n-gram draft assistance"
    ...
    # Log final metrics to wandb for vLLM/SGLang
    if (
        (args.use_vllm or args.use_sglang)
